...
with open(input_file, 'r') as fh:
    ...
    for line in fh:
        ...
        if isize is None:
            ...
            # build the 3d 'space'
            space = np.chararray((ss,ss,ss), itemsize=1)
...
def viewspace():
    for z in range(ss):
        for y in range(ss):
            for x in range(ss):
                print(space[x,y,z].decode("utf-8"), end='')
            print()
        print()
