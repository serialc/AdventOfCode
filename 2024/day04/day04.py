...
for y in range(sh):
    for x in range(sh):
        ...
        if y <= (sh - wlen) and x <= (sw - wlen):
            if np.all(
                surf[y : (y + wlen), x : (x + wlen)][np.identity(wlen, dtype=bool)] == w
            ):
                xfound += 1
        ...
        if y <= (sh - wlen) and x >= (wlen - 1):
            ...
            if np.all(
                surf[y : (y + wlen), (x - wlen + 1) : (x + 1)][
                    np.identity(wlen, dtype=bool)[::-1]
                ]
                == w
            ):
                xfound += 1
        ...
        if y >= (wlen - 1) and x >= (wlen - 1):
            if np.all(
                surf[(y - wlen + 1) : (y + 1), (x - wlen + 1) : (x + 1)][
                    np.identity(wlen, dtype=bool)
                ]
                == w[::-1]
            ):
                xfound += 1
